...
- **s m/t** - structure: body-statement skeleton agreement, **count-based** and
  brace-trimmed (the synthetic `{`/`}` are dropped, matching pdb_fetch's
...
cur / best / retries come from one mtime-cached query over the project's
`docs/binary_matching/match.db` (the same DB `match_db.py` / `match_score.py`
read, regenerated by every `rebuild.py`), keyed by the mangled name; structure
and locals are derived from the base/target rich index (which carries the
`statements` and `locals` match.db does not). The view headers and the build
popup show the same metrics. Refreshed on enter/save and after a build. Toggle
with `:Vostok hints`, or default off with `hints = false`.
